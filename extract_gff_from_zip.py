...
def get_file_from_zip(name : str, g_zip : str, g_dir : str):
    ...
    with zipfile.ZipFile(g_zip) as z_file:
        for z_name in z_file.namelist():
            ...
            #print(f"{z_dir} = {name}")
            if z_dir == name:
                if "_GeneCatalog_" in file or "filtered_proteins" in file or "filtered_genes" in file:
...
def check_missing_files(f_dir : str, g_dir : str, g_zip : str):
    ...
    for file in os.listdir(f_dir):
        if file.endswith(".fasta"):
            name = file.split(".")[0]
            if (not os.path.exists(f"{g_dir}{name}.gff") and
                not os.path.exists(f"{g_dir}{name}.gff3")):
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--fdir", dest="f_dir", help="Directory containing .fasta files")
    parser.add_argument("-g", "--gdir", dest="g_dir",
                        help="Directory containing .gff and .gff3 files")
    parser.add_argument("-z", "--zip", dest="g_zip",
                        help="Zip file containing .gff and .gff3 files")

    args = parser.parse_args(["-f", "/processing/jgi/nucl/", "-g", "/drive/gff/", "-z", "/home/menno/Downloads/annotation.zip"])
